...
public class KitManager {
    ...
    public boolean savekit(UUID uuid, int slot, ItemStack[] kit) {
        if (Bukkit.getPlayer(uuid) != null) {
            ...
            if (player != null) {
                ...
                if (notEmpty) {
                    // Проверка корректности брони в слотах
                    if (kit[36] != null) {
                    ...
                    player.sendMessage(ChatColor.GREEN + "Кит " + slot + " сохранен!");
    ...
    public boolean savePublicKit(Player player, String publickit, ItemStack[] kit) {
        ...
        if (notEmpty) {
            ...
            player.sendMessage(ChatColor.GREEN + "Публичный кит " + publickit + " сохранен!");
        ...
        if (Bukkit.getPlayer(uuid) != null) {
            ...
            if (player != null) {
                ...
                if (notEmpty) {
                    ...
                    player.sendMessage(ChatColor.GREEN + "Эндер-сундук " + slot + " сохранен!");
    ...
    public boolean loadKit(Player player, int slot) {
        return loadKitInternal(player, IDUtil.getPlayerKitId(player.getUniqueId(), slot), "Кит " + slot + " не существует!", false, () -> {
            BroadcastManager.get().broadcastPlayerLoadedPrivateKit(player);
            player.sendMessage(ChatColor.GREEN + "Кит " + slot + " загружен!");
    ...
    public boolean loadPublicKit(Player player, String id) {
        return loadKitInternal(player, IDUtil.getPublicKitId(id), "Кит не существует!", false, () -> {
            BroadcastManager.get().broadcastPlayerLoadedPublicKit(player);
            player.sendMessage(ChatColor.GREEN + "Публичный кит загружен!");
            player.sendMessage(ChatColor.GRAY + "Вы можете сохранить измененную версию этого кита, импортировав его в редакторе");
        });
    }
    ...
    public boolean loadEnderchest(Player player, int slot) {
        return loadKitInternal(player, IDUtil.getECId(player.getUniqueId(), slot), "Эндер-сундук " + slot + " не существует!", true, () -> {
            BroadcastManager.get().broadcastPlayerLoadedEnderChest(player);
            player.sendMessage(ChatColor.GREEN + "Эндер-сундук " + slot + " загружен!");
        });
    }
    ...
    public void loadPublicKitFromDB(String id) {
        ...
        if (!data.equalsIgnoreCase("error")) {
            try {
                ...
                plugin.getLogger().info("Ошибка загрузки публичного кита " + id);
            }
        }
    }
