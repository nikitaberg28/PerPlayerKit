            ...
            if (args.length == 1) {
                ...
                if (kitManager.hasKit(uuid, slot)) {
                    ...
                    if (kitManager.deleteKit(uuid, slot)) {
                        player.sendMessage(ChatColor.GREEN + "Кит " + slot + " удален!");
                        SoundManager.playSuccess(player);
                    } else {
                        player.sendMessage(ChatColor.RED + "Ошибка при удалении кита!");
                        SoundManager.playFailure(player);
                    }

                } else {
                    player.sendMessage(ChatColor.RED + "Кит " + slot + " не существует!");
                    SoundManager.playFailure(player);
                }
