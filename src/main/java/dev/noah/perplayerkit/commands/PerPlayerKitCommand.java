...
public class PerPlayerKitCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "about":
                sender.sendMessage(ChatColor.GREEN + "PerPlayerKit — это плагин, который позволяет игрокам иметь свои собственные киты.");
                return true;
            case "import":
                if (args.length < 2) {
                    sender.sendMessage(ChatColor.RED + "Укажите тип импорта!");
                    return true;
                }

                switch (args[1].toLowerCase()) {
                    case "kitsx":
                        sender.sendMessage(ChatColor.GREEN + "Запуск импорта...");
                        ...
                        sender.sendMessage(ChatColor.GREEN + "Попытка импорта данных KitsX завершена!");
