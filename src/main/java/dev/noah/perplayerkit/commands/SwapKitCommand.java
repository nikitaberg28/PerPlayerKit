...
public class SwapKitCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(ChatColor.RED + "Только игроки могут использовать это!");
            return true;
        }
        ...
        if (slot1 == null || slot2 == null) {
            player.sendMessage(ChatColor.RED + "Использование: /swapkit <слот1> <слот2>");
            player.sendMessage(ChatColor.RED + "Введите корректные числа");
            SoundManager.playFailure(player);
            return true;
        }
        ...
        if (!kitManager.hasKit(uuid, slot1)) {
            player.sendMessage(ChatColor.RED + "Кит " + slot1 + " не существует!");
            SoundManager.playFailure(player);
            return true;
        }

        if (!kitManager.hasKit(uuid, slot2)) {
            player.sendMessage(ChatColor.RED + "Кит " + slot2 + " не существует!");
            SoundManager.playFailure(player);
            return true;
        }
        ...
        player.sendMessage(ChatColor.GREEN + "Киты " + slot1 + " и " + slot2 + " были поменяны местами!");
